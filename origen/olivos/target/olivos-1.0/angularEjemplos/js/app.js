...
//	var urlBase="http://localhost:8080/aulafree";
	var urlBase="http://www.mijailaymara.com";
	...
	$scope.addTask = function addTask() {
		if($scope.taskName=="" || $scope.taskDesc=="" || $scope.taskPriority == "" || $scope.taskStatus == ""){
			alert(" Datos insuficientes! Llene todos los campos");
		}
		else{
	...
	// toggle selection for a given task by task id
	  $scope.toggleSelection = function toggleSelection(taskId) {
	    ...
	    // is newly selected
		  ...
		  success(function(data) {
			 alert("Tarea marca como completada");
			 $scope.tasks = data;
		    });
	...
	// Archive Completed Tasks
	  $scope.archiveTasks = function archiveTasks() {
		  ...
		  success(function(data) {
			  $scope.tasks = data;
		       alert("Tareas completadas archivadas");
		    });
	  };
	...
	function(){
         return {
             link: function (scope, element, attr) {
                 var msg = attr.ngConfirmClick || "Seguro?";
